...
class VoteyFormControlApplyDirective {
    ...
    observeFormControl() {
        ...
        this.eventsSubscription = this.formControl.events.subscribe(() => this.handleFormControlStateChange());
    }
...
class VoteyTextAreaComponent extends VoteyFormControlApplyDirective {
    ...
    errorTranslationKey = computed(() => {
        if (!this.hasError())
            return null;
        const [errorName] = Object.keys(this.formControl.errors ?? {});
        return errorName ? `ERRORS.${errorName.toUpperCase()}` : null;
    }, ...(ngDevMode ? [{ debugName: "errorTranslationKey" }] : /* istanbul ignore next */ []));
    focus(options) {
        if (this.effectiveDisabled())
            return;
