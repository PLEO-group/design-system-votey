...
- flow-skill-improver: Analizuj dłuższy przebieg pracy w bieżącym flow czatu i wskazuj, czy warto stworzyć nowy skill albo usprawnić użyte skille. Używaj przy retrospekcji zakończonego etapu pracy z AI. (file: skills/flow-skill-improver/SKILL.md)
...
- votey-design-system: Implementuj i reviewuj UI korzystające z `@pleodigital/design-system-votey` w `design-system-votey`, `wyborek-crm`, `votey-user-app` oraz innych potwierdzonych konsumentach paczki. Używaj przy przekładaniu handoffu z Figmy na kod, doborze tokenów, responsywności, assetów SVG, publicznego API paczki i lokalnych prymitywów Votey. Triggeruj dla zmian Angular/SCSS w CRM, React/Next/Tailwind w PWA, tokenów Votey, importów `@pleodigital/design-system-votey`, `@votey/*`, `tokens.angular.css`, `tokens.light.css`, `tokens.dark.css`, `tokens.tailwind.css`, klas `rv-*`, `provideVoteyDeviceDetection()` oraz implementacji z makiet Figmy w ekosystemie Votey. Pomiń dla projektów BoxEs, aplikacji bez tej paczki oraz samego odczytu Figmy bez implementacji w kodzie. (file: skills/votey-design-system/SKILL.md)
...
- Przed uruchomieniem formattera sprawdź konfigurację i wersję używaną przez repo. Formatuj tylko pliki należące do bieżącego zakresu i od razu sprawdź diff; jeśli formatter wprowadza niezwiązany churn albo niepoprawnie formatuje składnię frameworka, nie używaj go jako fallbacku i cofnij wyłącznie jego mechaniczne zmiany.
